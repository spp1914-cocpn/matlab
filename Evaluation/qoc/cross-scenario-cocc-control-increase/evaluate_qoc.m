prefix = 'cross-scenario-cocc-control-increase';
simTime = 180; % in seconds
numRepetitions = 100;
numNcs = 1; % one real ncs and plenty of mocks

%prefix = [prefix '-robust'];

names = {'300', '400', '500', '600'}; % indicate the number of total flows
name = names{end};
type = '';
%type='';
...
% and plot the actual packet delays during single run, starting from 140s
figure('Name', scenarioName, 'NumberTitle','off');
title('Actual Packet Delays (Single Run, 140 -180s)');

hold on;
scatter(actDelayTimes((actDelayTimes >= 140)), actPacketDelays(actDelayTimes >= 140), 'LineWidth', 2);
xlim([140 simTime]);
xticks([140:10:simTime]);
ylim([0 25]);
yticks([0:5:25]);
ylabel('Packet Delay (in milliseconds)');
xlabel('Simulation Time (in seconds)');
...
% plot a histogram of the actual packet delays in that run until 140s
figure('Name', scenarioName, 'NumberTitle','off');
title('Actual Packet Delays - Histogram (Single Run, 0-140s)');
hold on;
histogram(actPacketDelays(actDelayTimes < 140), 0:5:30);
ylabel('Count');
...
hold off;
 ...
 % plot a histogram of the actual packet delays in that run until 140s
figure('Name', scenarioName, 'NumberTitle','off');
title('Actual Packet Delays - Histogram (Single Run, 140 -180s)');
hold on;
histogram(actPacketDelays(actDelayTimes >= 140), 0:5:30);
ylabel('Count');
xlabel('Packet Delay (in milliseconds)');
